...
void QuaZip::close()
{
    ...
    switch (p->mode) {
        ...
        case mdAdd:
            p->zipError = zipClose(
                    p->zipFile_f, p->comment.isNull() ? NULL : p->commentCodec->fromUnicode(p->comment).constData());
            break;
        default:
...
QString QuaZip::getCurrentFileName() const
{
    ...
    if ((fakeThis->p->zipError
         = unzGetCurrentFileInfo64(p->unzFile_f, NULL, fileName.data(), fileName.size(), NULL, 0, NULL, 0))
        != UNZ_OK) {
        return QString();
    }
