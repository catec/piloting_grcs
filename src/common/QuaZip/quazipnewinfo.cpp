...
static void QuaZipNewInfo_setPermissions(
        QuaZipNewInfo* info, QFile::Permissions perm, bool isDir, bool isSymLink = false)
{
...
void QuaZipNewInfo::setFileNTFSTimes(const QString& fileName)
{
    ...
    if (!fi.exists()) {
        qWarning("QuaZipNewInfo::setFileNTFSTimes(): '%s' doesn't exist", fileName.toUtf8().constData());
        return;
    }
...
static void setNTFSTime(QByteArray& extra, const QDateTime& time, int position, int fineTicks)
{
        ...
        if (type == QUAZIP_EXTRA_NTFS_MAGIC) {
            ...
            while (i <= extra.size() - 4) {
                unsigned tag = static_cast<unsigned>(static_cast<unsigned char>(extra.at(i)))
                             | (static_cast<unsigned>(static_cast<unsigned char>(extra.at(i + 1))) << 8);
                i                += 2;
                unsigned tagsize  = static_cast<unsigned>(static_cast<unsigned char>(extra.at(i)))
                                 | (static_cast<unsigned>(static_cast<unsigned char>(extra.at(i + 1))) << 8);
                i += 2;
    ...
    if (timesPos == -1) {
        ...
        memmove(extra.data() + timesPos + 28, extra.data() + timesPos, extra.size() - 28 - timesPos);
        ntfsLength          += 28;
        // now set the tag data
        extra[timesPos]      = static_cast<char>(QUAZIP_EXTRA_NTFS_TIME_MAGIC);
        extra[timesPos + 1]  = static_cast<char>(QUAZIP_EXTRA_NTFS_TIME_MAGIC >> 8);
    ...
    quint64   ticks = (static_cast<qint64>(base.date().daysTo(utc.date())) * Q_INT64_C(86400000)
