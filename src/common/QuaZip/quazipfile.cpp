...
class QuaZipFilePrivate
{
  ...
  private:
    ...
    inline QuaZipFilePrivate(
            QuaZipFile* q, const QString& zipName, const QString& fileName, QuaZip::CaseSensitivity cs) :
            q(q), raw(false), writePos(0), uncompressedSize(0), crc(0), internal(true), zipError(UNZ_OK)
    {
        zip            = new QuaZip(zipName);
    ...
    inline QuaZipFilePrivate(QuaZipFile* q, QuaZip* zip) :
            q(q), zip(zip), raw(false), writePos(0), uncompressedSize(0), crc(0), internal(false), zipError(UNZ_OK)
    {}
    /// The destructor.
    inline ~QuaZipFilePrivate()
    {
...
QuaZipFile::QuaZipFile(QObject* parent) : QIODevice(parent), p(new QuaZipFilePrivate(this)) {}

QuaZipFile::QuaZipFile(const QString& zipName, QObject* parent) :
        QIODevice(parent), p(new QuaZipFilePrivate(this, zipName))
{}

QuaZipFile::QuaZipFile(const QString& zipName, const QString& fileName, QuaZip::CaseSensitivity cs, QObject* parent) :
        QIODevice(parent), p(new QuaZipFilePrivate(this, zipName, fileName, cs))
{}

QuaZipFile::QuaZipFile(QuaZip* zip, QObject* parent) : QIODevice(parent), p(new QuaZipFilePrivate(this, zip)) {}
        ...
        int                  strategy)
{
    ...
    if ((mode & WriteOnly) && !(mode & ReadOnly)) {
        ...
        p->setZipError(zipOpenNewFileInZip3_64(
                ...
                &info_z,
                info.extraLocal.constData(),
                info.extraLocal.length(),
                info.extraGlobal.constData(),
                info.extraGlobal.length(),
...
bool QuaZipFile::atEnd() const
{
    ...
    if (openMode() & ReadOnly) {
        ...
        return QIODevice::bytesAvailable() == 0 && unzeof(p->zip->getUnzFile()) == 1;
    } else {
        return true;
    }
}
